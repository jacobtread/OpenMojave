use super::{note::NOTE, prelude::*, scpt::SCPT, soun::SOUN};
use crate::esp::record::sub::{
    condition::CTDA, destruction::DestructionData, model::ModelData, object_bounds::ObjectBounds,
    script::Script,
};
...
impl Record for TERM {
    ...
    fn parse<'b>(parser: &mut RecordParser<'_, 'b>) -> Result<Self, RecordParseError<'b>> {
        ...
        let password_note: Option<TypedFormId<NOTE>> = parser.try_parse(SNAM)?;
...
impl RecordCollection for MenuItem {
    fn parse_next<'b>(
        ...
        let display_note: Option<TypedFormId<NOTE>> = parser.try_parse(INAM)?;
        let sub_menu: Option<TypedFormId<TERM>> = parser.try_parse(INAM)?;
        let embedded_script = Script::require_parse_next(parser)?;
